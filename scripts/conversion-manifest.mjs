 ...
 *
 * **Rimisurato il 2026-09-22, piu' tardi lo stesso giorno, fase 51 (D-51-09b):
 * sono ancora 44** = **36** dichiarate + **6** in attesa di un'altra fase +
 * **2** qui. Questa volta le cifre stanno ferme **perche' il conto non era in
 * gioco**, e la ragione e' diversa da entrambe le righe qui sopra: il piano
 * 51-06 non ha ne' aggiunto ne' tolto una pagina — **ne ha spostata una**, da
 * `/dashboard` a `/account`, con `git mv`.
 *
 * **Ed e' il caso che mostra perche' il censimento da solo non basta.** Una
 * pagina che cambia indirizzo lascia le cifre identiche e **rompe comunque il
 * gate**, perche' la voce del manifest continuerebbe a dichiarare il file
 * vecchio: il controllo D legge il *file dichiarato*, e un'entry che punta a un
 * file che non c'e' fa asserire al gate la cosa giusta sul file sbagliato — la
 * stessa cosa che e' successa questa mattina alle due entry cancellate, per la
 * strada opposta. Per questo la voce e' stata rinominata, **con il nuovo
 * percorso**, nello stesso commit dello spostamento.
 *
 * **Tre righe datate, e tre ragioni diverse per cui 44 non si muove**: la prima
 * perche' una pagina in meno ne compensava una in piu', la seconda per
 * coincidenza su entrambi i lati, la terza perche' nessuna pagina e' nata o
 * morta. A occhio le tre si leggono uguali — ed e' la dimostrazione di cio' che
 * questo blocco dice da due righe: quello che conta non e' il numero.
 *
...
export const CONVERTED = [
  ...
  [
    "/tickets", "src/app/(public)/tickets/page.tsx", "default",
    "riparazione autorizzata dal proprietario il 2026-08-22 — non un piano di fase. Superficie NUOVA, non una conversione: e' nata sui primitivi condivisi, quindi la claim non e' «e' stata portata» ma «non ha mai avuto altra forma». 'Whole' qui e' UN file, il file di rotta stesso: nessun loading, error o not-found le sta accanto, e la directory e' stata ELENCATA, non presunta; l'unico import locale e' zero — tutto cio' che monta viene dai primitivi gia' dichiarati. NON e' sulla lista chiusa wide del §4 e quindi default, che non e' un ripiego ma la risposta per ogni superficie su cui nessuno ha dovuto discutere. focus e' NON DISPONIBILE e non differito: il check E fallisce ogni superficie che dichiara la forma focus mentre monta una navigazione, e questa ne monta una. LA FRASE DELLA NAVIGAZIONE: monta la forma responsiva DIRETTAMENTE e dichiara nello stesso commit lo spazio della colonna al tier tablet, su un div che avvolge la shell con la navigazione come fratello, la utility copiata alla lettera dal gemello src/app/(public)/tickets/[id]/page.tsx:169. PERCHE' ESISTE, che e' la sola cosa che questa riga deve trasmettere: fino al 2026-08-22 il biglietto stava sul percorso critico della posta, e la posta poteva non arrivare mai in silenzio — la lista di soppressione del fornitore accetta la chiamata e non consegna. Questa pagina toglie il biglietto da quel percorso. LA COPY NON PROMETTE UN ACCOUNT, ed e' un vincolo del proprietario posto lo stesso giorno: l'acquisto da ospite dovra' essere possibile, quindi una frase che presupponesse una sessione diventerebbe falsa per una parte dei compratori. Dice dove sta il biglietto e che la mail non serve, mai come chi arriva viene riconosciuto. LA CAUTELA, e riguarda l'unico atto di questo prodotto che non ha annullamento: QUESTA PAGINA NON MOSTRA NESSUNA SEDE — zero occorrenze di una colonna di venue in ogni sua select, e la scelta e' scritta nel file. Il gemello /tickets/[id] mostra la sede al titolare sotto un predicato che DIVERGE da quello della pagina pubblica della serata, divergenza documentata li' e mai riconciliata; aggiungere qui una terza porta su quella stessa informazione sarebbe un allargamento deciso da una pagina di elenco. IL MODO DI RESA E' DICHIARATO E NON DERIVATO — force-dynamic, una occorrenza — a differenza del gemello, dove la riga mancante e' registrata come domanda dovuta al proprietario: qui non c'era nessuna decisione da rovesciare, la pagina nasce oggi e nasce dichiarata. LO STATO VUOTO E QUELLO D'ERRORE SONO DUE, e mai lo stesso: una lettura fallita dei biglietti e una degli acquisti in corso hanno ciascuna la propria frase, e il vuoto non parla mai a nome di una query che non ha risposto — su questa superficie dire «nessun biglietto» a chi ne ha uno sarebbe l'errore che la pagina esiste per impedire, commesso da noi invece che dalla posta. LA FRASE DEL NESSUN-ALLARGAMENTO, che e' la riga contro cui si legge il diff: nessuna colonna aggiunta, nessun vincolo toccato, nessuna policy scritta, nessun controllo di capability introdotto, nessun cancello nuovo su status — la lettura e' .eq(user_id, user.id) sotto la policy tickets_select_own che gia' esisteva, e la lettura degli acquisti in corso sotto pending_select_own, anch'essa gia' esistente",
  ],
  [
    "/tickets/order/[token]", "src/app/(public)/tickets/order/[token]/page.tsx", "focus",
    "piano 49-06 — superficie NUOVA, non una conversione: e' nata sui primitivi condivisi, quindi la claim non e' «e' stata portata» ma «non ha mai avuto altra forma». 'Whole' qui e' UN file, il file di rotta stesso: nessun loading, error o not-found le sta accanto, e la directory e' stata ELENCATA, non presunta; zero import locali — tutto cio' che monta viene dai primitivi gia' dichiarati. PERCHE' ESISTE: chi compra senza account non ha una password, quindi il gemello src/app/(public)/tickets/[id]/page.tsx:114-117 lo rimbalza a /login. I link della mail d'ordine (piano 49-05) arrivavano fin li' e finivano contro quel muro, e il codice allegato regge solo per chi non blocca le immagini: questa pagina e' la strada che percorre una persona che ha gia' pagato. LA CREDENZIALE E' UNA FIRMA HMAC NELL'INDIRIZZO, non una sessione, e la verifica avviene PRIMA di qualunque lettura. focus, e la lista chiusa del §4 e' stata ALLARGATA di una riga per decisione, con la ragione scritta accanto — non un rosso tolto in silenzio. La strada e' stata percorsa in tutte e due le direzioni e vale la pena registrarla: dichiarata focus, il check D ha arrossito perche' la rotta non era in lista; portata a default, il check E ha arrossito perche' la forma default RISERVA lo spazio di una navigazione che questa pagina non monta — e non montarla non e' un'omissione, e' il punto: la superficie si apre da una firma SENZA SESSIONE, e montare la navigazione significherebbe risolvere una sessione proprio sulla pagina la cui ragione e' che non serve. I due rossi insieme dicono qual e' la proprieta' vera che quella coppia sorveglia — una schermata a scopo unico che non monta navigazione — e questa la soddisfa esattamente, mentre la frase letterale della lista, «one card, one action», e' tirata: qui i riquadri sono uno PER BIGLIETTO, fino a sei. Detto invece che arrotondato. E' la continuazione di /payment/callback, che nella lista c'e' gia', per il lettore che quella pagina reindirizza quando chi ha comprato non ha un account. LA CAUTELA, e riguarda l'unico atto di questo prodotto che non ha annullamento: QUESTA PAGINA NON MOSTRA NESSUN LUOGO E NON NE LEGGE NESSUNO — zero colonne del genere in tutte e cinque le sue select, e nessun predicato di rivelazione importato, perche' su questa superficie la risposta e' *mai* (D-49-04, decisione del proprietario: l'indirizzo va solo a chi ha comprato, per mail, e la credenziale del biglietto non diventa una chiave verso di esso). Un uuid firmato si inoltra come si inoltra un biglietto — al portatore e' il caso progettato, D-49-03 — e con esso viaggerebbe il luogo. La ricerca di fase raccomandava l'opposto; D-49-04 e' piu' recente e piu' stretta e vince. NON E' PIU' UNA CLAIM SOLO SCRITTA: il controllo G di scripts/verify-venue-surfaces.mjs la misura con una spazzata negativa sul codice vivo e un'allow-list POSITIVA di ogni colonna selezionata, provato per mutazione il 2026-09-06 — una colonna nuova arrossisce anche se il suo nome non contiene nessun termine vietato. E NON MOSTRA NEMMENO L'INDIRIZZO DI POSTA di chi ha comprato: buyer_email non e' selezionato, stessa disciplina applicata a un dato personale su un link che si inoltra. IL MODO DI RESA E' DICHIARATO E NON DERIVATO — force-dynamic, una occorrenza — perche' la pagina mostra lo stato di un pagamento e il service worker non deve poterla servire stantia. GLI STATI VUOTO E D'ERRORE SONO SEI, e mai lo stesso: segreto di firma assente, lettura dell'ordine fallita, lettura dei biglietti fallita, pagamento in corso, scaduto o fallito, e pagato-senza-biglietti — ciascuno con la sua frase e la sua riga di log, perche' il progetto non ha error tracking e la frase diversa e' l'unica osservabilita' che esiste. LA FRASE DEL NESSUN-ALLARGAMENTO: nessuna colonna aggiunta, nessuna migration, nessuna policy scritta, nessun controllo di capability introdotto, nessun cancello nuovo su status — le letture passano dal client di servizio DOPO la verifica della firma, e il rifiuto e' notFound() senza distinguere firma invalida da ordine assente, perche' distinguerli farebbe della pagina un oracolo sugli identificativi in un repository che non ha rate limiting",
  ],
  [
    "/terms", "src/app/(public)/terms/page.tsx", "default",
    "pagina legale, pubblicata il 2026-09-21 per decisione del proprietario, prima del listing della 003 — superficie NUOVA, non una conversione: nata sui primitivi condivisi dentro src/components/legal/LegalPage.tsx, che e' l'unico import locale dei tre file di rotta legali e monta la navigazione responsiva DIRETTAMENTE, con il wrapper che dichiara lo spazio della colonna e AppNav come fratello — la forma di src/app/(public)/events/page.tsx. 'Whole' qui e' il file di rotta piu' quel componente condiviso: nessun loading, error o not-found accanto, directory ELENCATA. NON e' sulla lista chiusa wide del §4 e quindi default; focus e' NON DISPONIBILE perche' monta una navigazione, e perche' un testo di sette sezioni non e' una card con un'azione. COSA C'E' SCRITTO: biglietto al portatore (D-49-03), tetto per ordine (BUY-02), sede per mail a chi ha comprato (D-49-04), nessun rimborso salvo annullamento, 18+, foto e video alle serate, token drink non rimborsati. LA CAUTELA: nessuna sede, nessuna ragione sociale (scelta del proprietario, registrata nel docblock di LegalPage.tsx), solo info@resonatemotion.com; nessun professionista ha validato il testo e il file non lo finge — legal-compliance.md, debito aperto in .planning/todos/. LA FRASE DEL NESSUN-ALLARGAMENTO: nessuna query oltre getAccessContext() per la navigazione, nessuna colonna, nessuna migration, nessuna policy, nessun controllo di capability introdotto",
  ],
  [
    "/refunds", "src/app/(public)/refunds/page.tsx", "default",
    "pagina legale, pubblicata il 2026-09-21 per decisione del proprietario, prima del listing della 003 — superficie NUOVA, non una conversione: nata sui primitivi condivisi dentro src/components/legal/LegalPage.tsx, che e' l'unico import locale dei tre file di rotta legali e monta la navigazione responsiva DIRETTAMENTE, con il wrapper che dichiara lo spazio della colonna e AppNav come fratello — la forma di src/app/(public)/events/page.tsx. 'Whole' qui e' il file di rotta piu' quel componente condiviso: nessun loading, error o not-found accanto, directory ELENCATA. NON e' sulla lista chiusa wide del §4 e quindi default; focus e' NON DISPONIBILE perche' monta una navigazione, e perche' un testo di sette sezioni non e' una card con un'azione. COSA C'E' SCRITTO: nessun rimborso se non si viene; rimborso intero se annulliamo o spostiamo la data; i token drink non usati NON si rimborsano (il rimborso automatico e' uscito il 2026-08-20, DRK-01); un pagamento fallito non emette biglietti. LA CAUTELA: nessuna sede, nessuna ragione sociale (scelta del proprietario, registrata nel docblock di LegalPage.tsx), solo info@resonatemotion.com; nessun professionista ha validato il testo e il file non lo finge — legal-compliance.md, debito aperto in .planning/todos/. LA FRASE DEL NESSUN-ALLARGAMENTO: nessuna query oltre getAccessContext() per la navigazione, nessuna colonna, nessuna migration, nessuna policy, nessun controllo di capability introdotto",
  ],
  [
    "/privacy", "src/app/(public)/privacy/page.tsx", "default",
    "pagina legale, pubblicata il 2026-09-21 per decisione del proprietario, prima del listing della 003 — superficie NUOVA, non una conversione: nata sui primitivi condivisi dentro src/components/legal/LegalPage.tsx, che e' l'unico import locale dei tre file di rotta legali e monta la navigazione responsiva DIRETTAMENTE, con il wrapper che dichiara lo spazio della colonna e AppNav come fratello — la forma di src/app/(public)/events/page.tsx. 'Whole' qui e' il file di rotta piu' quel componente condiviso: nessun loading, error o not-found accanto, directory ELENCATA. NON e' sulla lista chiusa wide del §4 e quindi default; focus e' NON DISPONIBILE perche' monta una navigazione, e perche' un testo di sette sezioni non e' una card con un'azione. COSA C'E' SCRITTO: cosa si raccoglie e perche', dove sta (Supabase eu-west-1, Vercel dub1, SumUp, Resend, PostHog EU solo lato server), solo cookie tecnici quindi nessun banner, foto e video, diritti e cancellazione via info@. LA CAUTELA: nessuna sede, nessuna ragione sociale (scelta del proprietario, registrata nel docblock di LegalPage.tsx), solo info@resonatemotion.com; nessun professionista ha validato il testo e il file non lo finge — legal-compliance.md, debito aperto in .planning/todos/. LA FRASE DEL NESSUN-ALLARGAMENTO: nessuna query oltre getAccessContext() per la navigazione, nessuna colonna, nessuna migration, nessuna policy, nessun controllo di capability introdotto",
  ],
];
