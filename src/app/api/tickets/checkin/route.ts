...
} from "@/lib/door/judge-at-scan-time";
...
/**
 * `ScanTimeJudgement` and `judgeAtScanTime` lived here until 2026-09-22 and
 * moved, byte for byte, to `@/lib/door/judge-at-scan-time` so that the
 * guest-list route can ask the same question about the same person on the
 * same night. Nothing about how this route uses them changed: a queued report
 * from an account the role arm refused is still judged at `scannedAt`, never
 * at *now* (ASSIGN-03), and the five arms are still the five arms.
 */
...
function legacyStatusFor(outcome: DoorOutcome): {
  valid: boolean;
  status: string;
} {
  if (outcome.outcome === "recorded")
    return { valid: true, status: "checked_in" };
...
export async function POST(request: Request) {
  try {
    ...
    const isQueuedReport =
      source === "offline_sync" && deviceScannedAt !== null;
    ...
    if (namedNight === null) {
      return NextResponse.json(
        {
          outcome: "not_valid",
          reason: "no_party_selected",
          valid: false,
          status: "no_party_selected",
        },
    ...
    if (auth.ok) {
      ...
      judgement = await judgeAtScanTime(
        serviceClient,
        partyId,
        deviceScannedAt,
      );
    ...
    async function respond(
      outcome: DoorOutcome,
      row: Pick<ScanEventInsert, "ticket_id" | "subject_user_id" | "cause">,
      legacy: LegacyFields,
    ) {
        ...
        return NextResponse.json(
          {
            ...
            error: "Scan could not be recorded",
          },
          { status: 503 },
        );
      }
