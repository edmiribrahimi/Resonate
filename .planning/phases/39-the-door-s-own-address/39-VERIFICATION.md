...
| `src/lib/supabase/middleware.ts` | Module-load assertion over both addresses; `/door` in `protectedPrefixes` | ✓ VERIFIED | `DOOR_ADDRESSES = ["/admin/scanner", "/door"] as const` with a `for…of` asserting both `resolveRoute` binding and `assignmentOpenable` (`middleware.ts:188-210`), each throw naming the address. `protectedPrefixes` block: `"/dashboard", "/membership-card", "/attendance", "/admin", "/door"` — 5 entries, `/door` present |
