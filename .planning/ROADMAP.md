...
| **DBT-11** | **Chi carica da iPhone non trova un muro (Critical, media).** Una foto **HEIC** e un video **`.mov`** (QuickTime, spesso HEVC) si **accettano e si spogliano** — mai rifiutati — con lo stesso ordine «spoglia, poi scrivi» delle foto JPEG; l'uscita e' un formato che ogni browser legge (JPEG per le foto), la riga registra il tipo dell'oggetto scritto, `verify:media-strip` copre i nuovi tipi, e la prova per mutazione usa un `.heic` e un `.mov` veri con posizione attiva. **Il tetto e' 50 MB per file, dichiarato** (limite dello storage in produzione, misurato): l'uploader lo dice prima, e sopra il tetto **tenta una compressione nel browser** prima dell'invio; se non basta, rifiuta dicendo dimensione e tetto. *(Decisioni del proprietario, 2026-09-23: «se un utente con iphone carica un heic, non deve venire rifiutato»; «tetto dichiarato di 50mb, se qualcuno dovesse caricare un file piu grosso, proviamo a comprimerlo».)* |
