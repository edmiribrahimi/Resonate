...
| **DBT-14** | **La cover passa dal server ed e' 4:5 (Critical: sede segreta).** Il caricamento della cover non scrive piu' dal browser nel bucket pubblico: passa dal server con lo stripper di `strip-metadata.ts`, accetta **HEIC** da iPhone, ridimensiona, esce in JPEG; la policy di scrittura di `event-images` si chiude al solo server. Formato dichiarato **4:5** (la locandina del grafico, 1080×1350, nessun secondo file), mostrata **come locandina**: da telefono altezza massima ~55vh centrata con lo stesso file sfocato a riempire, da desktop a fianco delle informazioni, miniature quadrate dal centro; niente ritaglio di titolo e data. Le cover esistenti in produzione censite prima. |
