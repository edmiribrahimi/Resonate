...
- **Gate entropia dei codici**: Un codice scansionabile che concede l'ingresso deve resistere a un tentativo di indovinarlo. Oggi l'unico codice che la porta legge e' il **QR del biglietto**, firmato HMAC con `TICKET_SIGNING_SECRET`: la sua forza e' la firma, non la lunghezza. *(Fino al 2026-09-22 la porta leggeva anche il codice socio, coniato da `public.handle_new_user` con `extensions.gen_random_bytes` dal 2026-09-05; la fase 51 ha cancellato la colonna, il conio e la regex che lo accettava — `BARE_MEMBERSHIP_PATTERN` ha **zero occorrenze** in `src/`, e questo gate la citava ancora il 2026-09-23.)* Un nuovo identificatore d'accesso nasce da un CSPRNG o da una firma, mai da `Math.random()`. Vedi anche `access-gating.md`.

- **Gate la porta ha due credenziali, e non hanno la stessa forza di prova**: Si entra col **biglietto** — QR firmato, verificato offline dalla firma — **oppure per nome dalla guest list** scaricata sul telefono, che non porta firma e si trova o non si trova nella lista. Per questo l'asimmetria e' dichiarata nel codice: un biglietto non in cache **si ammette e si segnala**, un nome non in lista **non si trova** e l'avviso sullo schermo dice di far entrare e sistemare nel report (`guestListWarningText`, D-51-10). *(La seconda credenziale era il codice socio senza firma; la fase 51 lo ha cancellato — D-51-02, MEM-03 — e con lui il rifiuto offline sul codice sconosciuto.)* Nessuna modifica puo' allargare cio' che un nome in lista apre, e ogni nome aggiunto va attribuito (`ticketing-payments.md`, gate guest list).
- **Gate rate limit sulla verifica**: Una rotta che accetta un codice e risponde «valido / non valido» e' un oracolo di forza bruta finche' non ha rate limiting, e questo repo non ne ha (`access-gating.md`). L'esempio che questo gate citava e' stato **cancellato con la fase 51**; il gate resta, perche' la porta continua a leggere codici e la prossima rotta di verifica nasce sotto la stessa regola. Situazione che lo fa scattare: una rotta nuova che dica se un codice esiste, aggiunta senza dichiarare per iscritto che e' interrogabile senza costo.
...
- **Gate l'indirizzo che si scalda e' quello che si usera'**: Niente in questo prodotto precache un documento — il manifest porta chunk, CSS, font e file di `public/`: **zero HTML, zero rotte, zero payload RSC**. Ogni documento offline viene da una cache runtime `NetworkFirst` a **24 ore** e **32 voci**, calda solo per una visita online precedente, e **le chiavi di cache sono URL**: i due indirizzi della porta sono due voci indipendenti, e scaldarne uno **non** scalda l'altro. Quindi la riga del runbook e': *apri la porta, online, su quel telefono, quella sera, **all'indirizzo a cui quel telefono sara' mandato***. Situazione che lo fa scattare: un telefono che fa la porta da mesi al vecchio indirizzo, mandato al nuovo, radio spenta, cache fredda, davanti a una fila. Estende `Gate provato prima della porta`: non basta che il percorso sia stato provato, va provato **a quell'indirizzo**.
  ...
  **Le 24 ore sono una decisione, non un default (proprietario, 2026-08-11).** Chiesto se la porta meritasse una regola di cache piu' lunga: **no**. Una porta servita da una cache di ieri e' una porta stantia, e questo dominio tratta una superficie stantia come un rischio, non come una comodita'. **La conseguenza accettata e' che il riscaldamento non e' un passo di migrazione: e' un costo di ogni serata**, e questo gate non scade con la fase 39. Chi propone di allungare la finestra per evitarselo sta proponendo di scambiare un minuto di preparazione con una porta che puo' mentire: e' una decisione gia' presa, e nella direzione opposta.
...
- When weighing a membership code against a ticket: the ticket is signed and the code is not — do not treat them as equal proof
