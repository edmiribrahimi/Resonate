...
- **Gate entropia dei codici**: Un codice scansionabile che concede l'ingresso deve resistere a un tentativo di indovinarlo. **Questa riga diceva `src/utils/qr.ts:49`, e indicava codice morto**: quella funzione non aveva importatori ed e' stata rimossa il 2026-09-05. Il generatore vero e' il trigger `public.handle_new_user`, e **dal 2026-09-05 conia da `extensions.gen_random_bytes`** — 32 caratteri per 10, **2^50**, contro i 2^40 nominali del `random()` seminato di plpgsql. Dieci e' il massimo che `BARE_MEMBERSHIP_PATTERN` accetta (`ScannerClient.tsx:71`): un codice piu' lungo la porta lo rifiuta. **I quattro codici emessi prima non sono stati rigenerati** (`D-49-01`: sono in mano a persone reali), quindi la debolezza vecchia sopravvive su quei quattro. Vedi anche `access-gating.md`.

- **Gate la porta ha due credenziali, e la seconda non guarda chi sei**: Si entra col biglietto **oppure** col solo `membership_code`, e la seconda strada ammette **senza leggere ne' ruolo ne' stato** (`src/app/api/tickets/attendance/route.ts:145`). Un QR di membership **non porta firma** — e' un URL col codice dentro — mentre il biglietto e' HMAC-firmato: alla porta le due credenziali **non hanno la stessa forza di prova**, e per questo un codice che il roster non conosce si rifiuta offline mentre un biglietto no. **E' debito NON scelto** (`D-49-01`), non un difetto dimenticato: va nominato in `49-VERIFICATION.md` e nessuna modifica puo' allargare cio' che quel codice apre.
