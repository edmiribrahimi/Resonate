...
- **Gate sigla corrente**: Ogni materiale porta la sigla del format **come e' oggi**. Il sistema visivo e' stato disegnato quando il giovedi' era SunSet: i template delle storie del giovedi' vanno reintestati su `RMDB-BZ` / `RMDB-MR` prima di produrre. Vedi `production-calendar.md`, gate cambio nome con coda.
...
## Scouting delle sedi — i criteri di MotionLab

MotionLab e' l'unico format in cui **la sede e' il contenuto**: gallerie e spazi
espositivi, uno diverso ogni volta. I quattro criteri sono pesati, e servono a
**dire no in fretta**:

| Peso | Criterio | Perche' |
|---|---|---|
| **4 / 10** | Cornice espositiva | Museo, galleria, project space, fondazione. Meglio con una mostra in corso da attraversare. |
| **3 / 10** | Musica gia' di casa | Chi ha gia' fatto dj set o opening ha risolto permessi, vicinato e pubblico serale. |
| **1,5 / 10** | Impianto audio | Pesa poco: l'impianto si puo' portare. Pesa comunque: cambia costo e tempi in uno spazio con opere. |
| **1,5 / 10** | Agibilita' serale | Capienza reale e somministrazione la sera. **E' il criterio che elimina piu' candidati belli.** |

- **Gate scouting non e' contatto**: Lo scouting copre 184 spazi su quattro format, ed e' **tutto lavoro a tavolino: nessuno e' stato chiamato.** Una classifica non e' una disponibilita'. Non presentare mai uno spazio in lista come uno spazio acquisito.
- **Gate le tre domande**: Le colonne "da verificare" si chiudono con una telefonata, e sono sempre le stesse tre domande: **che impianto c'e', quanta gente ci sta davvero, e se un dj ospite puo' suonare.** Per Resonate se ne aggiunge una quarta, che screma piu' di tutte: **fino a che ora si puo' suonare** — le schede delle dimore dicono "deejay e musica dal vivo" pensando a un ricevimento che finisce all'una, non a una notte che finisce alle sei.
...
- When using a format sigla: use today's, not the one the template was built with
...
- When presenting a scouted space: state that it has not been contacted
